...
class ConvolutionalLayer:
    ...
    def forward(self, X):
        ...
        self.FCLs = np.tile(
            FullyConnectedLayer(self.in_channels * self.filter_size ** 2, self.out_channels, isfull=False),
            (out_height, out_width))

        for y in range(out_height):
            for x in range(out_width):
                self.FCLs[y, x].W = Param(self.W.value.reshape((-1, self.out_channels)),
                                          self.W.grad.reshape((-1, self.out_channels)));
                self.FCLs[y, x].B = Param(self.B.value.reshape((-1, self.out_channels)),
                                          self.B.grad.reshape((-1, self.out_channels)));
        ...
        for y in range(out_height):
            for x in range(out_width):
                result[:, y, x, :] = self.FCLs[y, x].forward(
                    self.X[:, y:y + self.filter_size, x:x + self.filter_size, :].reshape((batch_size, -1)))
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                result[:, y: y + self.filter_size, x: x + self.filter_size, :] += self.FCLs[y, x].backward(
                    d_out[:, y, x, :]).reshape(batch_size, self.filter_size, self.filter_size, channels)
...
class MaxPoolingLayer:
    ...
    def forward(self, X):
        ...
        assert (height - self.pool_size) % self.stride == 0, "height = %i, self.pool_size = %i, self.stride = %i" % (
                ...
                for y in range(out_height):
                    ...
                    for x in range(out_width):
                        ind = np.unravel_index(
                            np.argmax(self.X[m, i: i + self.pool_size, j: j + self.pool_size, k], axis=None),
